...
tokenizer = AutoTokenizer.from_pretrained(r"C:\Users\Owner\Downloads\meta-llamaLlama-3.2-1B",cache_dir=r"C:\Users\Owner\Downloads\meta-llamaLlama-3.2-1B")
tokenizer.pad_token = tokenizer.eos_token
tokenizer.padding_side = "right"

model = AutoModelForCausalLM.from_pretrained(
    r"C:\Users\Owner\Downloads\meta-llamaLlama-3.2-1B",
    return_dict=True,
...
with torch.no_grad():
    for i in tqdm(range(0, len(test_dataset))):
        batch_prompts = test_dataset[i]['text']
        model_inputs = tokenizer(batch_prompts, return_tensors='pt', padding=True)  # Remove .to('cuda')
        # model = model.cuda()  # Comment out this line if running on CPU
...
def eval_result(label_predict, test_type):
    ...
    print("accuracy_score" + '_' + test_type, acc)
